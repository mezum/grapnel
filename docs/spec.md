...
| `as` | なし | `"C"` や `"C-S"` のように C/M/S/W を `-` でつないだもの。この修飾キーを押している間、どのルールにも合わない入力は、この修飾キーを押したものとして送る (例: macOS の Cmd を Ctrl として使う) |
...
`press = "hold"` で、実装の手順がキー列 1 chord またはマウスボタン 1 つだけのとき、入力を押している間は出力も押したままにする。入力のキーリピートは出力のキーリピートになる (出力 chord の修飾キーはリピートのたびに押し直す)。同じキーを複数の入力で押しっぱなしにしているときは、最後の入力を離したときに離す。それ以外は `tap` として扱い、押した瞬間に手順を実行する。キーリピートでは、手順が入力系 (キー・文字・マウス・待機) だけなら再度実行し、それ以外は実行しない。
...
`as` を指定した修飾キーが有効な間、ルールに合わない入力 (パッドとジェスチャーを除く) は、実際に押している修飾キーに `as` の修飾キーを加えて送る。キーボードのキーとマウスボタンは入力を離すまで押したままにし、ホイールはその場でタップする。
