...
- フックの監視: Windows はコールバックが `LowLevelHooksTimeout` を超えたフックを黙って外す。2 秒ごとのタイマーで、前回から新しい入力があれば、印を付けた VK 0xFF のキーアップと、今のカーソル位置への絶対指定のマウス移動 (相対の移動量 0 はフックまで届かない) を送り (`hook::probe`)、次の回に両方のフックが呼ばれたかを見る。呼ばれていなければ (またはフックが無ければ) `Engine::reset` してから付け直す。実際の入力の後だけ送るので、スクリーンセーバーを遅らせるのは最大 2 秒。UIPI で送れなければ判定しない。付け直しの失敗は続く間 1 度だけ知らせる。
...
- 配色は `styles.css` の CSS 変数にまとめる。既定はライトで、`prefers-color-scheme: dark` か `<html data-theme="dark">` でダークになる (`data-theme="light"` はライトに固定)。テーマの選択は WebView の `localStorage` に保存する。
